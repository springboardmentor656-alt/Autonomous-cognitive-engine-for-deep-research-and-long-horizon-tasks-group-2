...
from agent.state import AgentState  # Added for type hint if needed
...
def agent_runnable(state: dict):
    """
    Wrapper for integration with LangGraph.
    """
    task = state.get("task", "")
    todos = state.get("todos", [])
    
    # Ideally this would be state-aware and emit tool calls, 
    # but for now we wrap the existing loop.
    result = react_loop(task, todos)
